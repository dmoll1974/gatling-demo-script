...
class TargetsIoHelper extends Simulation {
  ...
  def beforeSimulation() {
    if (testRunId != "DEBUG")
        TargetsIoClient.sendEvent(targetsIoUrl, "start", testRunId, baselineBuild, buildResultKey, dashboardName, productName)
  ...
  def afterSimulation() {
    ...
    if (testRunId != "DEBUG")
        TargetsIoClient.sendEvent(targetsIoUrl, "end", testRunId, baselineBuild, buildResultKey, dashboardName, productName)
